...
public class SerrureRed : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Olaf")
        {
            for (int i = 0; i < 3; i++)
            {
                if (olafIconManager.listIcons[i].sprite == keyRed)
                {
        ...
        else if (collision.gameObject.tag == "Erik")
        {
            for (int i = 0; i < 3; i++)
            {
                if (erikIconManager.listIcons[i].sprite == keyRed)
                {
                    animPorte.Play("Porte");
                    col.enabled = false;
        ...
        else if (collision.gameObject.tag == "Baleog")
        {
            for (int i = 0; i < 3; i++)
            {
                if (baleogIconManager.listIcons[i].sprite == keyRed)
                {
                    animPorte.Play("Porte");
                    col.enabled = false;
